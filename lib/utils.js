...
async function getClientDetails(clientNumberOrId) {
    ...
    try {
        ...
        if (clientNumberOrId.toString().includes('@')) {
            console.log('Looking up by email:', clientNumberOrId);
            const { data: emailData, error: emailError } = await db
                .from('emails')
                .select('company_id')
                .eq('email', clientNumberOrId)
                .limit(1);
            ...
            if (emailData && emailData.length > 0) {
                ...
                console.log('Found company_id from email:', companyId);
            } else {
