...
/**
 * Resolve the `emails.email_id` for a client's contact address.
 *
 * `projects.email_id` is guarded by trg_projects_check_company_client, which
 * rejects a row whose email belongs to a different company than its client.
 * So the lookup is scoped to the company owning `clientId` whenever that
 * company can be found; an address we cannot place returns null, leaving both
 * email_id and (via trg_projects_set_person_from_email_fn) person_id empty.
 *
...
async function resolveEmailId(clientId, companyEmail) {
    if (!db || !db.from || !companyEmail) return null;

    const email = String(companyEmail).toLowerCase().trim();
    ...
    try {
        ...
        let query = db.from('emails').select('email_id').eq('company_email', email).limit(1);
        if (companyId !== null) {
            query = query.eq('company_id', companyId);
        }
        ...
        console.warn('resolveEmailId failed (non-fatal):', err.message);
        return null;
    }
}
