...
/**
 * Version number the next snapshot of this proposal will carry.
 *
 * Read from the proposal's own pointer rather than by counting history rows, so
 * that a storage path can be built *before* the version row exists — the
 * documents for v3 must be uploaded to `.../v3/` before there is a v3 to hang
 * them on.
 *
 * A proposal that predates versioning has current_version 0, so its first
 * snapshot is v1 — the same number the backfill gave it.
 */
function nextVersionNo(proposalRow) {
    return (Number(proposalRow && proposalRow.current_version) || 0) + 1;
}

/**
 * Storage folder holding one version's documents.
 *
 * Versions get their own folder because `uploadProposalFiles` writes with
 * `upsert: true`: sharing a path would mean regenerating a proposal silently
 * overwrites the document an earlier version still points at.
 */
function versionStoragePath(clientFolderName, offerNumber, versionNo) {
    return `${clientFolderName}/${offerNumber}/v${versionNo}`;
}

/**
 * Append a version to a proposal's history and advance its pointer.
 *
 * `proposalRow` must be the row as it stands *after* the write being recorded —
 * a version is a snapshot of committed state, not of an intention.
 *
 * `project_id` is what separates a draft version from a ready one. It is null
 * on every version cut before the proposal was marked ready, and set from that
 * version onwards: marking ready is the moment Lidia supplies the project
 * number, so a version carrying one is a proposal that has become a project.
 *
 * Version numbers are allocated from `current_version`, which is not atomic
 * across concurrent writers; the UNIQUE (proposal_id, version_no) constraint is
 * what actually prevents two snapshots claiming the same number, and a loser
 * surfaces as a failed insert rather than a silently mangled history.
 *
 * @param {Record<string, any>} proposalRow
 * @param {{ changeType: 'create'|'regenerate'|'ready', actor?: string|null }} opts
 * @returns {Promise<any|null>} the version row, or null if Supabase is absent
 */
async function createProposalVersion(proposalRow, { changeType, actor = null } = {}) {
    if (!db || !db.from) {
        throw new Error('Supabase not configured — cannot record a proposal version.');
    }
    if (!proposalRow || !proposalRow.id) {
        throw new Error('createProposalVersion requires a saved proposal row (with id).');
    }

    const versionNo = nextVersionNo(proposalRow);

    const version = {
        proposal_id: proposalRow.id,
        offer_number: proposalRow.offer_number,
        version_no: versionNo,
        change_type: changeType,
        project_id: proposalRow.project_id || null,
        proposal_status: proposalRow.proposal_status || null,
        services: proposalRow.services || null,
        pricing: proposalRow.pricing || null,
        total_price: proposalRow.total_price ?? null,
        document_url: proposalRow.document_url || null,
    ...
    if (error) {
        throw new Error(`Supabase version insert failed: ${error.message}`);
    }

    const { error: pointerError } = await db
        .from('proposals')
        .update({ current_version: versionNo })
        .eq('id', proposalRow.id);

    if (pointerError) {
        // The version itself is committed; a stale pointer only means the next
        // snapshot would collide on the unique constraint, which is loud.
        console.warn('⚠️  Version recorded but current_version not advanced:', pointerError.message);
    }

    console.log(`Proposal ${proposalRow.offer_number} → v${versionNo} (${changeType})`);
    return (data && data[0]) || null;
}

/**
 * Full version history for a proposal, newest first.
 */
async function listProposalVersions(offerNumber) {
    if (!db || !db.from) {
        throw new Error('Supabase not configured — SUPABASE_URL and SUPABASE_SERVICE_KEY (or their NEXT_PUBLIC_ equivalents) must be set.');
    }
    ...
    const { data, error } = await db
        .from('proposal_versions')
        .select('id, offer_number, version_no, change_type, project_id, proposal_status, total_price, document_url, created_by, created_at')
        .eq('offer_number', offerNumber)
        .order('version_no', { ascending: false });

    if (error) {
        throw new Error(`Supabase version list failed: ${error.message}`);
    }
