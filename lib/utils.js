...
async function uploadProposalFiles(docxBuffer, pdfBuffer, storagePath) {
    ...
    const [docxUpload, pdfUpload] = await Promise.all([
        db.storage.from(bucket).upload(docxPath, docxBuffer, {
            contentType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
            upsert: true,
        }),
        db.storage.from(bucket).upload(pdfPath, pdfBuffer, {
    ...
    const [docxSigned, pdfSigned] = await Promise.all([
        db.storage.from(bucket).createSignedUrl(docxPath, 60 * 60 * 24 * 7),
        db.storage.from(bucket).createSignedUrl(pdfPath,  60 * 60 * 24 * 7),
    ]);
...
module.exports = { getClientDetails, save_proposal_detail, getNextOfferNumber, uploadProposalFiles };
