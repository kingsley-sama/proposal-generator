...
class TemplateDocxProposalGenerator {
  ...
  // ── Image XML with black border + inner padding ────────────────
  // Wraps the image inside a single-cell table so there is visible padding
  // between the black border and the image itself.

  generateImageXml(rId, widthPx, heightPx, imageIndex) {
    // Convert pixels → EMUs (1 px at 96 dpi = 9525 EMU)
    const pxToEmu = 9525;
    const cx = widthPx * pxToEmu;
    const cy = heightPx * pxToEmu;

    const docPrId = 100 + imageIndex;

    // Table cell padding in twips (1 mm ≈ 57 twips, 4 mm ≈ 227 twips)
    const cellPad = 227;

    // Table width in twips (image width in EMU → twips: 1 twip = 635 EMU)
    // Add padding on both sides
    const tableWidthTwips = Math.round(cx / 635) + cellPad * 2;

    // Border: 4 (half-pt units → 0.5 pt thin line)
    ...
    return (
      // Centered paragraph with a single-cell table
      `<w:tbl>` +
        `<w:tblPr>` +
          ...
          `<w:tblCellMar>` +
            `<w:top w:w="${cellPad}" w:type="dxa"/>` +
            `<w:left w:w="${cellPad}" w:type="dxa"/>` +
            `<w:bottom w:w="${cellPad}" w:type="dxa"/>` +
            `<w:right w:w="${cellPad}" w:type="dxa"/>` +
          `</w:tblCellMar>` +
        `</w:tblPr>` +
        `<w:tr>` +
          `<w:tc>` +
