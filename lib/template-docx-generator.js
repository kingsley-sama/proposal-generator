...
class TemplateDocxProposalGenerator {
  ...
  generateFullDescriptionXml(items, url = '') {
    ...
    const traverse = (nodes, level = 0) => {
      nodes.forEach(node => {
        const text = (typeof node === 'string' ? node : (node.text || '')).trimEnd();
    ...
    // {@fullDescriptionXml} replaces the surrounding <w:p>. Returning '' deletes
    // the only paragraph in the cell, which Word renders as a giant blank cell
    // that overflows to the next page. Always return at least one paragraph so
    // the cell has valid OOXML and a normal one-line height.
  ...
  buildTemplateData() {
    ...
    const discountValueNum = pricing.discount ? Number(pricing.discount.value) || 0 : 0;
    const discountAmountNum = pricing.discount ? this.parsePriceToNumber(pricing.discount.amount) : 0;
    const hasDiscount = !!(pricing.discount && (discountValueNum > 0 || discountAmountNum > 0));
    const discountInfo = hasDiscount ? pricing.discount : null;
    const deliveryDays = projectInfo.deliveryTime || '7-10';
    ...
    const globalBuildingType = projectInfo.projectType || null;
    const customBuildingTypeLabel = projectInfo.customProjectType || null;

    const templateServices = services.map((s, index, arr) => {
      const isLast = index === arr.length - 1;

      // tiers layout:
      //   - Always: leading { text: '' } + 'Preisstaffelung:' header + tier rows
      //   - Non-last services: trailing { text: '' } spacer row
      //   - Last service: no trailing spacer
      //   - Services without pricingTiers: no tier rows emitted
      const tiers = (s.pricingTiers && s.pricingTiers.length)
        ? [
            { text: '' },
            { text: 'Preisstaffelung:' },
            ...s.pricingTiers.map(t => ({ text: t.label || t.text || '' })),
            ...(isLast ? [] : [{ text: '' }]),
          ]
        : [];

      // Resolve sub_name for exterior services: use whatever the client sent, but if it's
      // still the default placeholder (or missing), fall back to the per-instance building
      // type if set, otherwise to the global building type.
      let subName = s.sub_name || '';
      const isExterior = s.name === '3D-Außenvisualisierung Bodenperspektive' ||
                         s.name === '3D-Außenvisualisierung Vogelperspektive';
      const effectiveBuildingType = s.buildingType || globalBuildingType;
      if (isExterior && (!subName || subName === '(Projekt mit XX Wohneinheiten)') && effectiveBuildingType) {
        const resolvedType = effectiveBuildingType === 'Custom'
          ? (customBuildingTypeLabel || 'Custom')
          : (buildingTypeLabels[effectiveBuildingType] || effectiveBuildingType);
        subName = `(${resolvedType})`;
      }
    ...
    return {
      ...
      images_section_title: templateImages.length > 0 ? 'Empfohlene Perspektiven Außen' : '',
      ...
      hasDiscount,
      discountDescription: discountInfo ? (discountInfo.description || '') : '',
      ...
      p_three: terms.p_three || (services.some(s => s.name && s.name.includes('360° Tour')) ? italicData.p_three : ''),
  ...
  async generate(outputPath) {
    try {
      ...
      // ── Enable automatic hyphenation (Silbentrennung) ─────────────
      // settings.xml already sets <w:autoHyphenation/>, but the template's
      // docDefaults and Normal style carry <w:suppressAutoHyphens w:val="true"/>,
      // which excludes every paragraph from hyphenation — that's why Word never
      // hyphenated despite the flag. Strip the suppression and keep the
      // document-level flag as a guard in case the template gets replaced.
      let stylesXml = zip.file('word/styles.xml').asText();
      stylesXml = stylesXml.replace(/<w:suppressAutoHyphens[^>]*\/>/g, '');
      zip.file('word/styles.xml', stylesXml);

      let settingsXml = zip.file('word/settings.xml').asText();
      if (!settingsXml.includes('<w:autoHyphenation')) {
        const hyphenFlag = '<w:autoHyphenation w:val="true"/>';
        settingsXml = settingsXml.includes('<w:defaultTabStop')
          ? settingsXml.replace('<w:defaultTabStop', `${hyphenFlag}<w:defaultTabStop`)
          : settingsXml.replace('</w:settings>', `${hyphenFlag}</w:settings>`);
      ...
      // Word picks the hyphenation dictionary per run language; a few template
      // runs are tagged en-US/bg-BG, which would skip German hyphenation rules.
      docXml = docXml.replace(/<w:lang w:val="(?:en-US|bg-BG)"\/>/g, '<w:lang w:val="de-DE"/>');

      // ── Drop the services-row minimum height. ─────────────────────
      // The template's services <w:tr> sets <w:trHeight w:val="1275"
      // w:hRule="atLeast"/>, forcing every row to be at least ~2.25 cm
      // tall. Rows with multi-bullet descriptions exceed that anyway,
      // but a row with an empty/short description hits the floor and
      // renders as a huge blank cell. Removing the element lets the
      // row size to its actual content. The val="1275" attribute is
      // unique to this row in the template.
      docXml = docXml.replace(
        /<w:trHeight w:val="1275" w:hRule="atLeast"\/>/g,
        ''
      );

      // ── Make {#hasSubName} delete its parent paragraph when false. ─
      // The template places {#hasSubName}{sub_name}{/hasSubName} inside
      // its own <w:p>. When the condition is false, docxtemplater
      // strips the inner runs but leaves the empty paragraph behind,
      // which adds an extra line to the name cell and forces the row
      // taller. The {-w:p tag}content{/} syntax tells docxtemplater to
      // remove the entire parent <w:p> when the tag is falsy.
      docXml = docXml.replace(
        /\{#hasSubName\}\{sub_name\}\{\/hasSubName\}/g,
        '{-w:p hasSubName}{sub_name}{/}'
      );
      ...
      return { success: true, filePath: outputPath || null, buffer };
    } catch (error) {
