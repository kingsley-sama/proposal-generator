...
class TemplateDocxProposalGenerator {
  ...
  createBulletXml(text, level = 0, isLink = false, url = '') {
    ...
    // Level 0: render a true bullet using a manual bullet character (•) so the
    // output is never affected by the template's numbering definitions (numId=1
    // might be a numbered list after a template update).
    // Level 1+: keep using Word's numbering (sub-items / children).
    if (level === 0) {
      const bulletRPr =
        `<w:rPr>` +
      ...
      const bulletChar = isLink ? '' :
        `<w:r>${bulletRPr}<w:t>&#xB7;</w:t></w:r>` +
        ...
        getSize(imgBuffer) {
          try {
            ...
            if (!dims || !dims.width || !dims.height) {
              console.warn('⚠️ image-size returned invalid dimensions, using fallback');
              return [500, 350];
            }
            const maxW = 500;
            const maxH = 700;
            let w = dims.width;
            let h = dims.height;
            // Scale down to fit within maxW
            if (w > maxW) {
              h = Math.round(h * (maxW / w));
              w = maxW;
            }
            // Scale down to fit within maxH (for tall portrait images)
            if (h > maxH) {
            ...
            console.warn('⚠️ image-size failed, using fallback 500×350:', err.message);
            return [500, 350];
          }
        },
      };
