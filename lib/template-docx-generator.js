...
class TemplateDocxProposalGenerator {
    // Generate WordML for clickable hyperlink
    generateLinkXml(url) {
      ...
      return referenceLink ? referenceLink.linkXml : '';
    }
  constructor(data = {}) {
    this.data = data;
  }
  ...
  buildTemplateData() {
    ...
    const templateServices = services.map(s => {
      ...
      // If there is a link, add it as a top-level bullet (not a child)
      
      if (s.link) {
        description.push({
          text: '', // Leave text empty so only linkXml is rendered
          linkXml: this.generateLinkXml(s.link),
          link: s.link
        });
      }
      return {
