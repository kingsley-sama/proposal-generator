...
class TemplateDocxProposalGenerator {
  ...
  buildTemplateData() {
    ...
    const templateServices = services.map((s, index, arr) => {
      ...
      //   - Services without pricingTiers: non-last gets a single trailing { text: '' },
      //     last gets an empty array
      ...
      // still the default placeholder (or missing), fall back to the global building type.
      ...
      if (isExterior && (!subName || subName === '(Projekt mit XX Wohneinheiten)') && globalBuildingType) {
        const resolvedType = globalBuildingType === 'Custom'
          ? (customBuildingTypeLabel || 'Custom')
          : (buildingTypeLabels[globalBuildingType] || globalBuildingType);
